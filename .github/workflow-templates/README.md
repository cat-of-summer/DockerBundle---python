...
Свой `.sha256` рядом с бинарём не публикуется: он лежал бы на том же хосте, что и файл,
и подделывался бы вместе с ним, а целостность передачи закрывает HTTPS. Смысл есть в
другом — закрепить конкретные байты. GitHub печатает digest рядом с каждым ассетом; его
можно передать установщику, и тогда перезалитый релиз честно уронит сборку:

```
... | sh -s -- vX.Y.Z --dir "$RUNNER_TEMP/bin" --sha256 sha256:2603c64c...
```
