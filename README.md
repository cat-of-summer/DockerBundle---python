...
Каталог установки — `--dir`, иначе `$RUNNER_TEMP` под GitHub Actions, иначе
`~/.local/bin`. Флаг `--sha256` закрепляет конкретные байты: digest печатает сам GitHub
рядом с каждым ассетом релиза, и при перезалитом релизе установка честно упадёт.
